...
export function Nav() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-[#FAFAF8]/90 backdrop-blur-sm" style={{ height: '56px' }}>
        ...
        <nav className="hidden md:flex items-center gap-7">
          {NAV_LINKS.map(({ href, label }) => (
            <a
              key={href}
              href={href}
              className="font-sans text-[13px] text-secondary hover:text-primary transition-colors duration-150"
            >
              {label}
            </a>
          ))}
      ...
      <div
        className={`md:hidden border-t border-border bg-[#FAFAF8] overflow-hidden transition-all duration-200 ${
          open ? 'max-h-64' : 'max-h-0'
        }`}
      >
        <div className="max-w-[1100px] mx-auto px-8 py-5 flex flex-col gap-4">
          {NAV_LINKS.map(({ href, label }) => (
            <a
              key={href}
              href={href}
